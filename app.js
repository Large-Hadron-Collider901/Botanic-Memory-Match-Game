...
document.addEventListener("DOMContentLoaded", function () {
    ...
    Array.from(imgs).forEach(img =>
        img.addEventListener("click", flipCard) // for each image, listen for clicks!
    )
});
//createBoard function
function createBoard(grid, array) {
    popup.style.display = "none"; // removes the popup
    array.forEach((arr, index) => { // loops through the image array
...
function arrangeCard() { // shuffles the deck
    cardArray.sort(() => 0.5 - Math.random())
}
// flip Card function
function flipCard() { // listens for clicks
    ...
    if (cardsId.length === 2) { // repeat process when the second image is clicked and then we will check to see if they are the same image
...
function checkForMatch() {
    ...
    if (cardsSelected[0] === cardsSelected[1] && firstCard !== secondCard) { // checks if the first value in cardsArray is equal to the second and if it is not the same image that was clicked twice.
        ...
        imgs[secondCard].setAttribute("src", "assets/card.jpg"); alert("wrong, please try again"); imgs[firstCard].classList.remove("flip"); imgs[secondCard].classList.remove("flip"); // if the images do not match, removes the flip class and the cards are turned over
    }
...
function checkWon() {
    if (cardsWon == cardArray.length / 2) { // checks if the value of cardsWon is equal to the deck array divided by 2
        alert("You won")
